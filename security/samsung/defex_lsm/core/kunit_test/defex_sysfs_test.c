...
extern unsigned char defex_packed_rules[];
 ...
 *
 * The method reads the defex_packed_rules policy array and find path and file
...
static void find_paths(struct rule_item_struct *node, char *current_path, size_t path_len)
{
	...
	if (node->next_file) {
		find_paths(GET_ITEM_PTR(node->next_file), current_path, path_len);
	}
	...
	if (node->next_level) {
		find_paths(GET_ITEM_PTR(node->next_level), current_path, path_len);
	}
}
...
static void find_rules_for_test(void)
{
	...
	find_paths(GET_ITEM_PTR(base->next_level), path, 0);

	pr_info("kunit defex_sysfs_test: Path results:");
	pr_info("kunit defex_sysfs_test: first_file: %s", first_file);
	pr_info("kunit defex_sysfs_test: second_file: %s", second_file);
	pr_info("kunit defex_sysfs_test: existing_directory_no_features: %s",
		existing_directory_no_features);
	pr_info("kunit defex_sysfs_test: existing_directory_path_open: %s",
		existing_directory_path_open);
	pr_info("kunit defex_sysfs_test: existing_directory_path_write: %s",
...
static void lookup_tree_test(struct kunit *test)
{
	...
	if (strlen(first_file) > 0 && strlen(second_file) > 0) {
		...
		KUNIT_EXPECT_EQ(test, DEFEX_INTEGRITY_KUNIT_FAIL, lookup_tree(first_file, first_file_attr, file_two));
...
static void lookup_dir_test(struct kunit *test)
{
	...
	struct rule_item_struct *policy_base = (struct rule_item_struct *)defex_packed_rules;
	...
	KUNIT_EXPECT_EQ(test, NULL, lookup_dir(NULL, NULL, 0, 0));
	...
	if (strlen(existing_directory_no_features) > 0) {
		...
		do {
			...
			policy_item = lookup_dir(policy_base, path, size, 0);
	...
	KUNIT_EXPECT_EQ(test, NULL, lookup_dir(policy_base, DUMMY_DIR, strlen(DUMMY_DIR), 0));
...
static void defex_init_sysfs_test(struct kunit *test)
{
	/* __init function */
	KUNIT_SUCCEED(test);
}
...
{
#if defined(DEFEX_USE_PACKED_RULES) && defined(DEFEX_RAMDISK_ENABLE)
	struct rule_item_struct *base_struct = (struct rule_item_struct *)defex_packed_rules;
...
static int defex_sysfs_test_init(struct kunit *test)
{
#if defined(DEFEX_USE_PACKED_RULES) && defined(DEFEX_RAMDISK_ENABLE)
...
static void defex_sysfs_test_exit(struct kunit *test)
{
}

static struct kunit_case defex_sysfs_test_cases[] = {
	/* TEST FUNC DEFINES */
	...
	KUNIT_CASE(defex_init_sysfs_test),
